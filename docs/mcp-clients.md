...
Не добавляй сюда `write_file`/`patch_file`/`execute_code`/`delete_safe` — это пишущие/потенциально опасные операции, для них confirmation полезен.
...
- `execute_code`/`write_file`/`patch_file`/`delete_safe`/`purge_trash` — могут менять диск и запускать код. Включай confirmation на стороне клиента, не добавляй в `autoApprove`.
- `gofer` имеет полный доступ к проекту и через sandbox — к остальной файловой системе. Изоляция — задача клиента.
