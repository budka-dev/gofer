...
- ~16 read-only MCP-инструментов: hybrid search, symbols/refs/callers, compact read (skeleton/function_context/bundle), batch, index ops. Не замена FS/grep/git хоста.
...
Pet-проект, активная разработка. Сфокусирован как **read-only index-search MCP**: не FS/grep/git, не мутации, не sandbox, не LSP. Только индекс + компактное чтение; ответ собирает хост-агент. API инструментов и схемы конфигов могут меняться без обещаний совместимости.
